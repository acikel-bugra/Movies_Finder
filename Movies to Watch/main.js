...
function genResultCard(result) {
    ...
    apiHelper.processTMDB(result, rCard)
...
async function renderDetails(info, card, restype) {
    ...
    function getPG(relDates) {
        ...
        if (relDates !== undefined && relDates.results !== undefined) {
            certs = [].concat.apply([], relDates.results.map(date => date.release_dates.map(rd => rd.certification))) 
        .filter(item => item !== "")  
        .filter( onlyUnique ) 
        .map(item => { 
            item.replaceAll("+", ""); 
        ...
        .filter(item => typeof item === 'number' || normalratings.includes(item) || item.includes('PG-')) 
        .sort() 
        .sort((a, b) => a - b) 
        } else {
            certs = []
...
imgUtil.containCover = (imgArr) => { 
    if (imgArr === undefined || imgArr.length === 0) { return "cover" } 
